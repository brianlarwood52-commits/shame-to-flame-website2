    ...
    <div className="animate-fade-in min-h-screen pt-16">
      ...
      <section className="py-10 sm:py-12 lg:py-16 bg-gradient-to-br from-sky-50/90 to-flame-50/90 dark:from-sky-900/30 dark:to-flame-900/30 backdrop-blur-md">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Book className="h-12 w-12 sm:h-14 sm:w-14 lg:h-16 lg:w-16 text-flame-500 mx-auto mb-4 sm:mb-6" />
      ...
      {/* Search Form */}
      <section className="py-6 sm:py-8 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <div className="flex-1">
                <input
                  ...
                  placeholder="Search for Bible verses, books, or topics..."
                  className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-flame-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                />
              </div>
              <button
                type="submit"
                disabled={loading || !searchTerm.trim()}
                className="px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-flame-600 hover:bg-flame-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors duration-200 flex items-center justify-center"
              >
                {loading ? (
          ...
          {/* Search Info */}
          {totalResults > 0 && (
            <div className="mb-4 sm:mb-6 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
          ...
          {error && (
            <div className="mb-4 sm:mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg p-3 sm:p-4">
              <div className="flex items-start space-x-2 sm:space-x-3">
                <AlertCircle className="h-4 w-4 sm:h-5 sm:w-5 text-red-600 dark:text-red-400 mt-0.5 sm:mt-1 flex-shrink-0" />
                <div>
                  <p className="text-sm sm:text-base text-red-800 dark:text-red-200 font-medium">Search Error</p>
          ...
          {loading && (
            <div className="text-center py-8 sm:py-12">
              <Loader2 className="h-6 w-6 sm:h-8 sm:w-8 animate-spin text-flame-500 mx-auto mb-3 sm:mb-4" />
              <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">Searching Internet Archive...</p>
            </div>
          )}
          ...
          {!loading && results.length > 0 && (
            <div className="space-y-4 sm:space-y-6">
              {results.map((result, index) => (
                <div key={result.identifier} className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-xl p-4 sm:p-6 shadow-lg border border-white/20 dark:border-gray-700/50">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div className="flex-1">
                      <h3 className="font-serif text-lg sm:text-xl font-semibold text-gray-800 dark:text-white mb-2">
                        {result.title}
                      </h3>
                        ...
                        <div className="mb-2 sm:mb-3">
                          ...
                          <div className="flex flex-wrap gap-1 sm:gap-2">
                            {result.subject.slice(0, 5).map((subject, idx) => (
                              <span
                                key={idx}
                                className="text-xs bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 px-2 py-0.5 sm:py-1 rounded-full"
                              >
                                {subject}
                              </span>
                            ))}
                    ...
                    <div className="sm:ml-4 flex-shrink-0">
                      <a
                        ...
                        className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-sm sm:text-base bg-flame-600 hover:bg-flame-700 text-white font-medium rounded-lg transition-colors duration-200"
                      >
                        View
      ...
      {/* API Information */}
      <section className="py-6 sm:py-8 bg-gradient-to-r from-sky-50/90 to-flame-50/90 dark:from-sky-900/30 dark:to-flame-900/30 backdrop-blur-md">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/20 dark:border-gray-700/50">
            <h3 className="font-serif text-base sm:text-lg font-semibold text-gray-800 dark:text-white mb-3 sm:mb-4">
